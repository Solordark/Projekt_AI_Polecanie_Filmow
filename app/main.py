...
def polec_filmy(id_filmu, df, ile_polecic=5):
    ...
    for rzad in df.itertuples():  # dziwna architektura pandy wymaga iterowania po tuplach
        ...
        pod_studio = cosinus_dla_list(
            film_docelowy["production_companies"], rzad.production_companies
        )
        ...
        pod_budzet = podobienstwo_liczbowe(
            film_docelowy["budget"], rzad.budget, min_budzet, max_budzet
        )
        pod_rok = podobienstwo_liczbowe(
            film_docelowy["release_date"], rzad.release_date, min_rok, max_rok
        )
...
"""
Funkcje do dodania:
1. Łatwiejsze wyszukiwanie filmow (ich ID)
2. Możliwość wpisywania ich w input a nie w kodzie
3. Możliwość wpisania wielu filmów początkowych
